...
# ---------------- WINDOWS DIRECT USB PRINTER ----------------
# Uses Windows SetupAPI to write directly to USB printer device
# No Zadig/libusbK needed - works with native Windows Usbprint.sys driver

class WindowsDirectUSBPrinter:
    """
    Send ESC/POS commands directly to USB printer using Windows device path.
    This uses the native Windows Usbprint.sys driver - no Zadig needed.
    """

    # Windows SetupAPI constants
    DIGCF_PRESENT = 0x02
    DIGCF_DEVICEINTERFACE = 0x10

    # USB Printer class GUID: {28d78fad-5a12-11d1-ae5b-0000f803a8c2}
    GUID_DEVINTERFACE_USBPRINT = ctypes.c_buffer(
        b'\xad\x8f\xd7\x28\x12\x5a\xd1\x11\xae\x5b\x00\x00\xf8\x03\xa8\xc2'
    ...
    def find_usb_printer_path(self, vid, pid):
        """
        Find the Windows device path for a USB printer by VID/PID.
        Returns path like: \\?\usb#vid_0fe6&pid_811e#...#{guid}
        """
        if PLATFORM != "Windows":
            return None

        try:
            # Use SetupAPI to enumerate USB printer devices
            setupapi = ctypes.windll.setupapi

            # GUID for USB Printer class
            ...
            guid = GUID()
            guid.Data1 = 0x28d78fad
            guid.Data2 = 0x5a12
            guid.Data3 = 0x11d1
            guid.Data4 = (ctypes.c_ubyte * 8)(0xae, 0x5b, 0x00, 0x00, 0xf8, 0x03, 0xa8, 0xc2)
            ...
            if hDevInfo == -1:
                logger.error("SetupDiGetClassDevsW failed")
                return None
            ...
            vid_str = f"VID_{vid:04X}".upper()
            pid_str = f"PID_{pid:04X}".upper()
            ...
            while True:
                ...
                class SP_DEVICE_INTERFACE_DETAIL_DATA(ctypes.Structure):
                    _fields_ = [
                        ("cbSize", ctypes.c_ulong),
                        ("DevicePath", ctypes.c_wchar * (required_size.value - 4))
                    ]
                ...
                if result:
                    ...
                    logger.info(f"Found USB printer device: {device_path}")

                    # Check if this device matches our VID/PID
                    path_upper = device_path.upper()
                    if vid_str in path_upper and pid_str in path_upper:
                        logger.info(f"Matched VID/PID! Device path: {device_path}")
            ...
            logger.error(f"Error finding USB printer path: {e}")
            return None

    def configure(self, vid, pid):
        """Configure by finding the device path for given VID/PID."""
        self._vid = vid
        self._pid = pid
        self._device_path = self.find_usb_printer_path(vid, pid)

        if self._device_path:
            logger.info(f"Windows Direct USB configured: {self._device_path}")
        else:
            logger.warning(f"No USB printer device found for VID={hex(vid)} PID={hex(pid)}")

    def send_raw(self, data):
        """Send raw bytes directly to the USB printer device."""
        if not self._device_path:
            raise RuntimeError("USB printer device path not found")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Wrote {len(data)} bytes to USB printer")
                return True
            finally:
                win32file.CloseHandle(handle)

        except Exception as e:
            raise RuntimeError(f"Failed to write to USB printer: {e}")

    def test_print(self):
        """Send a test print."""
        commands = b"\x1B\x40"  # Initialize
        commands += b"=== GLAMIRIS TEST ===\n"
        commands += b"Direct USB Print OK!\n"
        commands += b"\n\n"
    ...
    def is_available(self):
        """Check if device path was found."""
        return self._device_path is not None


# Global Windows Direct USB printer instance
windows_direct_usb = WindowsDirectUSBPrinter() if PLATFORM == "Windows" else None
...
def test_direct_usb():
    ...
    # On Windows, try direct USB device path first (uses native Usbprint.sys - no Zadig needed)
    if PLATFORM == "Windows" and windows_direct_usb is not None:
        result["steps"].append("Windows detected - trying direct USB device path (no Zadig needed)")
        try:
            windows_direct_usb.configure(vid, pid)
            if windows_direct_usb.is_available():
                result["steps"].append(f"Found device path: {windows_direct_usb._device_path}")
                windows_direct_usb.test_print()
                result["status"] = "ok"
                result["backend"] = "windows_direct_usb"
                result["message"] = "Test print sent via Windows direct USB device path"
                return jsonify(result)
            else:
                result["steps"].append("Direct USB device path not found")
        except Exception as e:
            result["steps"].append(f"Direct USB failed: {e}")

        # Fallback to Windows print spooler
        result["steps"].append("Trying Windows print spooler fallback...")
        if HAS_WIN32PRINT:
            windows_raw_printer.configure(vid, pid)
            if windows_raw_printer.is_available():
                try:
                    windows_raw_printer.test_print()
                    result["status"] = "ok"
                    result["backend"] = "windows_spooler"
                    result["message"] = "Test print sent via Windows print spooler"
                    return jsonify(result)
                except Exception as e:
                    result["steps"].append(f"Windows spooler test failed: {e}")
            else:
                result["steps"].append("Windows spooler printer not found for this VID/PID")

        result["steps"].append("Falling back to libusb (requires Zadig driver on Windows)")
